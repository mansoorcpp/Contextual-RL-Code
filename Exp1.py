...
class SkipAndGrayResizeWrapper(gym.Wrapper):
    ...
    """
    def __init__(self, env: gym.Env, context: Dict, shape=(84,84), skip = 4, num_stack: int = 4):
        super().__init__(env)
        self.shape = (int(shape[0]), int(shape[1]))
        self.num_stack = int(num_stack)
        # copy context so each wrapper has its own dict (no shared mutation)
    ...
    def _get_obs_frames(self):
        if len(self.frames) < self.num_stack:
            if len(self.frames) == 0:
                pad = np.zeros((self.shape[0], self.shape[1]), dtype=np.float32)
            else:
                pad = self.frames[0]
            arr = [pad] * (self.num_stack - len(self.frames)) + list(self.frames)
        else:
            arr = list(self.frames)
        stacked = np.stack(arr, axis=0).astype(np.float32)
        return stacked

    # ---- reset/step ----
    def reset(self, **kwargs):
        """
        Inject context via Gymnasium `options` dict so underlying CARLMarioEnv sees it.
        SB3's VecEnv will call reset(seed=..., options=...) sometimes; we ensure options["context"] is set.
        """
        # optionally randomize noise_seed per episode (keeps it local)
        self.context["noise_seed"] = random.randint(0, 2**31 - 1)

        # Prepare options dict, preserving any existing options in kwargs
        options = kwargs.pop("options", {}) or {}
        # pass context WITHOUT context_id into options (env may not expect context_id)
        context_to_pass = {k: v for k, v in self.context.items() if k != "context_id"}
        options["context"] = context_to_pass
        kwargs["options"] = options

        # Respect seed if provided; forward to base env
    ...
    def step(self, action):
        ...
        for _ in range(self.skip):
            ...
            # incremental reward for this frame
            completed = float(info.get("completed", 0.0))
            reward_delta = completed - getattr(self, "_prev_completed", 0.0)
            self._prev_completed = completed
            total_reward += reward_delta

            # process frames for stacking
            frame = self._process(obs)
            self.frames.append(frame)
...
if __name__ == "__main__":
    ...
    env_fns = [make_env_fn(i) for i in range(max_contexts + 1)]
    vec_env = DummyVecEnv(env_fns)
    vec_env = VecMonitor(vec_env)  # tracks per-subenv episode reward/length
