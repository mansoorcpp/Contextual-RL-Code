...
# CONFIG
# =========================
max_contexts = 15  # number of discrete inertia values / training envs
num_stack = 4
...
def make_carl_contexts_for_inertia(idx,inertia: float) -> Dict[int, Dict]:
    """
    Build a single CARLMario context for this inertia.
    One map per env.
    """
    ...
    ctx = {
        ...
        "level_index": int(idx),   # fixed random map for this env
        "noise_seed": int(np.random.randint(0, 2**31 - 1)),
        "mario_state": int(base["mario_state"]),
        "mario_inertia": float(inertia),
    }

    return {0: ctx}   # only context_id=0 exists

def make_carl_contexts_for_inertia1(inertia: float) -> Dict[int, Dict]:
    """
    Build a single CARLMario context for this inertia.
    One map per env.
    """
    ...
    ctx = {
        "level_width": int(base["level_width"]),
        "level_index": int(np.random.randint(0, 14)),   # fixed random map for this env
        "noise_seed": int(np.random.randint(0, 2**31 - 1)),
        "mario_state": int(base["mario_state"]),
        "mario_inertia": float(inertia),
    }
...
def context_to_vec_raw(ctx: Dict) -> np.ndarray:
    """
    Raw context -> 20D vector: [level_width, 14x level_index OH, noise_seed,
...
# Normalization settings for 19D context vector
_CONTEXT_NORM = {
    "level_width_min": 0.0,
...
class SkipAndGrayResizeWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        - Delegate to underlying env.reset() (CARLMarioEnv).
        - After reset, sync wrapper context with CARL's context (one map per env).
        - Build stacked-frame observation and attach normalized context vector.
        """
        self.episode_counter += 1
        self._prev_completed = 0.0
        self._ep_reward_accum = 0.0

        base_env = self.env  # this is CARLMarioEnv

        # 1) Call CARLMarioEnv.reset so it sets context and resets MarioEnv
        ...
        # 2) If this is CARLMarioEnv, sync wrapper.context from CARL's context
        if isinstance(base_env, CARLMarioEnv):
            carl_env: CARLMarioEnv = base_env

            # After reset, CARLMarioEnv.context should be a dict, not None
            carl_ctx_obj = getattr(carl_env, "context", None)
            if carl_ctx_obj is not None:
                carl_ctx = dict(carl_ctx_obj)
                # keep our logging-only context_id, copy the rest
            ...
            self._current_carl_context_id = getattr(carl_env, "context_id", 0)

            debug_info["carl_context_id"] = self._current_carl_context_id
            debug_info["carl_context"] = carl_ctx
            ...
            print("[RESET] Wrapper context:", dict(self.context))
            print(
                f"[RESET] CARLMarioEnv context_id={self._current_carl_context_id}, "
                f"context={carl_ctx}"
            )
            print(
                "[RESET] MarioEnv state: "
            ...
            print(f"\n[RESET] Episode {self.episode_counter}")
            print("[RESET] Wrapper context (env is not CARLMarioEnv):", self.context)
        ...
        # 3) Build stacked-frame observation
...
def make_train_env_fn(
    ...
    def _init():
        ...
        carl_contexts = make_carl_contexts_for_inertia(idx,inertia)  
...
def make_eval_env_fn(
    ...
    """
    def _init():
        carl_contexts = make_carl_contexts_for_inertia1(inertia)
...
def evaluate_model(
    ...
    For each inertia:
      ...
      - log mean reward and mean completion
    ...
    for idx, inertia in enumerate(inertias):
        ...
        env_fn = make_eval_env_fn(inertia)
        ...
        for ep in range(episodes_per_context):
            ...
            completions.append(last_completed)
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            for ep_i, (r, c) in enumerate(zip(total_rewards, completions), start=1):
                writer.writerow([ep_i, r, c])
