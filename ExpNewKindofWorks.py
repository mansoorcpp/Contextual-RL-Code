...
def context_to_vec_raw(ctx: Dict) -> np.ndarray:
    """
    Raw context -> 16D vector:
      [15D one-hot level_index, 1D inertia]
    """
    level_idx_oh = one_hot(ctx["level_index"], N_LEVEL_VARIANTS)  # shape (15,)
    inertia = np.array([float(ctx["mario_inertia"])], dtype=np.float32)  # shape (1,)
    return np.concatenate([level_idx_oh, inertia], axis=0)  # shape (16,)
...
def normalize_context_vec(raw_vec: np.ndarray) -> np.ndarray:
    """
    Normalize 16D context vector [15D one-hot level_index, 1D inertia].
    - one-hot part is left unchanged
    - inertia is mapped from [0.5, 1.5] -> [0, 1]
    """
    expected_dim = N_LEVEL_VARIANTS + 1  # 15 + 1 = 16
    assert raw_vec.shape[0] == expected_dim, \
        f"expected context len {expected_dim}, got {raw_vec.shape[0]}"
    out = np.empty_like(raw_vec, dtype=np.float32)

    # one-hot level_index unchanged
    out[:N_LEVEL_VARIANTS] = raw_vec[:N_LEVEL_VARIANTS]

    # inertia normalized to [0, 1]
    inertia_raw = raw_vec[N_LEVEL_VARIANTS]
    out[N_LEVEL_VARIANTS] = np.clip(
        (inertia_raw - _CONTEXT_NORM["inertia_min"])
...
class SkipAndGrayResizeWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        level_change_interval: int | None = 50,
    ):
        super().__init__(env)
    ...
    def step(self, action):
        ...
        - Call underlying env.step(action) up to `self.skip` times
        - Sum the environment rewards over those steps
        - Stop early if terminated or truncated
        - Update the frame stack with each observed frame
        - Return a dict observation {"frames", "context"} and the summed reward
        """
        total_reward = 0.0
        ...
        for _ in range(self.skip):
            ...
            # --- use the environment's own reward ---
            r = float(reward)
            total_reward += r
            self._ep_reward_accum += r

            # --- image processing / frame stack ---
            frame = self._process(obs)
            self.frames.append(frame)
        ...
        # Build dict observation: stacked frames + normalized context vector
        obs_dict = {
            "frames": self._get_obs_frames(),
            "context": normalize_context_vec(context_to_vec_raw(self.context)),
        }
        ...
        if (terminated or truncated) and self.logger is not None:
            ...
            self._ep_reward_accum = 0.0
            self._prev_completed = 0.0  # no longer used for reward, but keep clean

        return obs_dict, float(total_reward), terminated, truncated, last_info

# -------------------------------------------------------
# cGate/Hadamard Extractor
#-----------------------------------------------------

class HadamardFeatureExtractor(BaseFeaturesExtractor):
    """
    Feature extractor with Hadamard (element-wise) fusion of
    frame features and context features.

    - CNN over stacked frames -> frame_feat  (B, n_flat)
    - MLP over context vector -> ctx_feat    (B, n_flat)
    - Fused = frame_feat * ctx_feat          (B, n_flat)
    - Final MLP: Linear(n_flat, features_dim) + ReLU
    """
    ...
    def __init__(self, observation_space: gym.spaces.Dict, features_dim: int = 512):
        ...
        # compute n_flat from dummy forward
        with torch.no_grad():
        ...
        # --- final fusion layer ---
        # Hadamard product yields n_flat-dim vector
    ...
    def forward(self, observations):
        frames = observations["frames"].float()   # (B, C, H, W)
        context = observations["context"].float() # (B, context_dim)

        frame_feat = self.cnn(frames)             # (B, n_flat)
        ctx_feat = self.context_proj(context)     # (B, n_flat)

        # Hadamard (element-wise) fusion
        fused = frame_feat * ctx_feat             # (B, n_flat)
...
# FEATURE EXTRACTOR
# =========================
class SimpleConcatFeatureExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Dict, features_dim: int = 512):
        super().__init__(observation_space, features_dim)
...
def make_train_env_fn(
    ...
    def _init():
        ...
        wrapped = SkipAndGrayResizeWrapper(
            ...
            num_stack=num_stack,
            logger=logger,
            level_change_interval=25,  # map changes every 25 episodes
        )
        return wrapped
...
if __name__ == "__main__":
    ...
    policy_kwargs = dict(
        features_extractor_class=HadamardFeatureExtractor,
