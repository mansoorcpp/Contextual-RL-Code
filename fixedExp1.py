...
N_LEVEL_VARIANTS = 14  # how many different maps per inertia/env


def make_carl_contexts_for_inertia(inertia: float) -> Dict[int, Dict]:
    """
    Build multiple CARLMario contexts sharing the same inertia but with
    different noise_seed and level_index.
    """
    base = get_context_features()
    contexts: Dict[int, Dict] = {}

    for cid in range(N_LEVEL_VARIANTS):
        contexts[cid] = {
            "level_width": int(base["level_width"]),
            "level_index": int(np.random.randint(0, 14)),             # 0..13
...
def context_sampler(idx: int, max_ctx: int = max_contexts) -> Dict:
    ...
    inertia = 0.5 + idx * aggregation  # spans [0.5, 1.5) as idx increases
...
def context_to_vec_raw(ctx: Dict) -> np.ndarray:
    ...
    return np.concatenate(
        [level_width, level_idx_oh, noise_seed, mario_state_oh, inertia], axis=0
    )
...
def normalize_context_vec(raw_vec: np.ndarray) -> np.ndarray:
    ...
    # noise seed scaled
    out[15] = np.clip(
        raw_vec[15] / _CONTEXT_NORM["noise_seed_max"],
        0.0,
        1.0,
    )
    # mario_state one-hot
    ...
    # inertia mapped
    out[19] = np.clip(
        (raw_vec[19] - _CONTEXT_NORM["inertia_min"])
...
class SkipAndGrayResizeWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        # -------- case 1: not CARLMarioEnv (fallback) --------
        if not isinstance(base_env, CARLMarioEnv):
            ...
            if not self._initialized:
                ...
                if (
                    self.level_change_interval
                    and (self.episode_counter - 1) % self.level_change_interval == 0
                ):
                    # ensure we know how many contexts exist
                    if self._num_carl_contexts == 0:
                        self._num_carl_contexts = len(getattr(carl_env, "contexts", {}))

                    # move to next context id (cyclic)
                    if self._current_carl_context_id is None:
                        self._current_carl_context_id = getattr(
                            carl_env, "context_id", 0
                        )
                    else:
                ...
                if isinstance(ret2, tuple) and len(ret2) == 2:
                    obs, info = ret2
                else:
                    obs, info = ret2, {}
        ...
        if isinstance(self.env, CARLMarioEnv):
            carl_env = self.env
            ...
            print("[RESET] Wrapper context:", debug_info["wrapper_context"])
            print(
                f"[RESET] CARLMarioEnv context_id={current_id}, "
                f"context={carl_current_ctx}"
            )
            print(
...
def make_train_env_fn(
    ...
    def _init():
        ...
        wrapped = SkipAndGrayResizeWrapper(
            ...
            num_stack=num_stack,
            logger=logger,
            level_change_interval=10,  # map changes every 10 episodes
        )
        return wrapped
...
if __name__ == "__main__":
    ...
    env_fns = [make_train_env_fn(i, logger=logger) for i in range(max_contexts + 1)]
